...
def make_sine_kernels(sz=9, ks=[(1.0,0.0)], freqs=[1.0]):
    ...
    xs, ys = grid_for_sz(sz)
    sine_kernels = []
    for phase in [xs*k[0] + ys*k[1] for k in ks]:
        sine_kernels += [np.sin(freq * phase) for freq in freqs]
    return kernel_list_to_tf(sine_kernels)

def make_gabor_kernels(sz=9, ks=[(1.0,0.0)], freqs=[1.0]):
    sine_kernels = make_sine_kernels(sz=sz, ks=ks, freqs=freqs)
    gauss_kernels = make_gauss_kernels(sz=sz, sigmas=freqs)
    gauss_kernels = np.repeat(gauss_kernels, 
                              len(sine_kernels) // len(gauss_kernels), axis=0)
    logging.info(f"Gaussien kernel shape = {gauss_kernels.shape}, "
...
def show_filter_bank(filter_bank:tf.Tensor, rows=1):
    ...
    for n in range(filter_count):
        ...
        if rows > 1:
            axs[n // rows][n % per_row].imshow(filter, cmap='plasma')
        else:
