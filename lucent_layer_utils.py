    ...
    try:
            ...
            if len(parts) >= 2:
                ...
                if hasattr(model, mixed_name):
                    ...
                    if hasattr(mixed_module, branch_name):
                        ...
                        if hasattr(branch_module, 'conv') and hasattr(branch_module.conv, 'out_channels'):
        ...
        # Method 2: Handle conv2d style names (e.g., "conv2d0_pre_relu_conv")
            ...
            if hasattr(model, conv_name):
                ...
                if hasattr(conv_module, 'conv') and hasattr(conv_module.conv, 'out_channels'):
        ...
        for name, module in model.named_modules():
            if lucent_layer_name in name or name.endswith(lucent_layer_name.split('_')[-1]):
                if hasattr(module, "out_channels"):
                    return module.out_channels
...
def get_visualizable_layers(model, include_detailed_info=False):
    ...
    skip_patterns = [
        'pool', 'dropout', 'fc', 'avgpool', 'adaptiveavgpool', 'flatten',
        'relu', 'leakyrelu', 'elu', 'gelu', 'softmax', 'sigmoid', 'tanh',
        'batchnorm', 'layernorm', 'instancenorm', 'groupnorm',
        'aux', 'logits', 'classifier', 'features', 'bn', 'norm'
    ]
