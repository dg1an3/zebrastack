...
def gabor_function(x, y, sigma_x=1.0, sigma_y=1.0, theta=0, lambda_freq=1.0, psi=0, gamma=1.0):
    """
    ...
    # Gaussian envelope
    gaussian = np.exp(-0.5 * ((x_theta**2 / sigma_x**2) + (gamma**2 * y_theta**2 / sigma_y**2)))
    ...
    try:
        ...
        for i in range(size):
            for j in range(size):
                ...
                gabor_weights[i, j] = gabor_function(x, y, sigma_x, sigma_y, theta, lambda_freq, psi, gamma)
        ...
        def gabor_weighted_obj(model):
            ...
            if len(layer_acts.shape) != 4:  # Should be [batch, channels, height, width]
                print(f"Warning: Layer {layer_name} has unexpected shape {layer_acts.shape}")
            ...
            if actual_h != size or actual_w != size:
                ...
                cropped_weights = gabor_weights[weight_h_start:weight_h_end, weight_w_start:weight_w_end]
            else:
                cropped_weights = gabor_weights
            
            # Convert weights to tensor on same device as activations
            weights_tensor = torch.tensor(cropped_weights, dtype=offset_region.dtype, device=offset_region.device)
        ...
        print(f"Error creating Gabor-weighted objective: {e}")
        return None


def create_gabor_preset_objectives(layer_name, channel_idx, preset="edge_detector", **kwargs):
    """
...
def visualize_gabor_weights(size=7, sigma_x=1.0, sigma_y=1.0, theta=0, lambda_freq=2.0, psi=0, gamma=1.0):
    """
...
def create_multi_orientation_gabor_objective(
    ...
    **gabor_kwargs
    ...
    for i, (theta, weight) in enumerate(zip(orientations, weights)):
        print(f"Creating Gabor objective {i+1}/{len(orientations)} with orientation {theta:.2f} rad")
...
if __name__ == "__main__":
    ...
    presets = ["edge_detector", "vertical_edges", "diagonal_edges", "texture_fine", "blob_detector"]
    ...
    print(f"Weight range: [{weights_horizontal.min():.3f}, {weights_horizontal.max():.3f}]")
